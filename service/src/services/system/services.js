...
exports.printcode =async (posId,arrayPrint) =>{
  try {
    

    const print = await systemRepository.findPrinterConfiguration(posId);
    if(!print){
      throw new AppError("No se encuentra configuración de impresora", 404);
      return false;
    }
    ...
    client.connect(print.puerto,print.ip, () => {
    ...
    client.on("error", async (err) => {
      console.error("Error:", `${err.message} no se logró enviar la impresión a ${JSON.stringify(print)}`);
      return false;
    });
    return true;
  } catch (error) {
